...
lipo_obelisk_v10.py
───────────────────
v10 — raw-axis pipeline with **differentiable resampling** (adaptive α).

This file is a direct counterpart to `lipo_obelisk_baseline_v10.py`. The only
intentional difference is the resampling step:

  Baseline v10:  CPU MONAI Spacingd(mode=3, cubic) — fixed target spacing
  v10 (this):    in-model GPU bilinear grid_sample — target = α ⊙ s_ref
                 where α is learnable (default init α = 1)

Everything else is shared with the baseline:
- Raw NIfTI orientation (no Orientationd(RAS))
- Dataset fingerprint from raw headers
- CropForegroundd with Otsu (MRI-appropriate)
- ZScore on RAW image, fg_mode=gt_zero, train AND val identical (Plan A)
- Same GPU patch sampler (pos-sample guarantees patch contains tumor)
- Same nnU-Net-strict augmenter
- OOF: predictions warped back to raw original space via R_α^-1 (inverse
  grid_sample), Dice computed against raw NIfTI label

α optimization variants (CLI flags)
────────────────────────────────────
  default        : backprop α through the training loss
  --freeze_alpha : α frozen at 1.0 — sanity check that the pipeline matches
                   baseline_v10 when α is locked
  --darts        : DARTS-style bi-level optimization, finite-difference α
                   updates on val_dice_orig every K epochs

Reference spacing for α (CLI --alpha_ref)
──────────────────────────────────────────
  median (default): s_ref = dataset median target spacing → α=1 reduces to baseline
  unit            : s_ref = (1, 1, 1) — fully decoupled from dataset statistics
  orig            : s_ref = each case's own raw spacing → α=1 is identity (no resample)
"""
...
class ZScoreNormalizeForegroundd(MapTransform):
    ...
    def __call__(self, data):
        ...
        for key in self.keys:
            ...
            # IN-PLACE write to preserve MetaTensor wrapper (and its affine).
            # MetaTensor is a Tensor subclass, so `torch.is_tensor` returns True
            # for both; using copy_ keeps d[key] pointing to the same MetaTensor.
            if torch.is_tensor(img):
...
class StoreRawSpacingAndTargetd(MapTransform):
    """Read raw spacing from MetaTensor affine and store it together with the
    fixed target spacing (s_ref) for the model to consume.

    For v10 we do NOT apply Spacingd in CPU transforms — the resampling happens
    in the model. This transform makes the raw spacing and target spacing
    available as tensors in the data dict.
    """

    def __init__(self, keys, target_spacing, allow_missing_keys=False):
        super().__init__(keys, allow_missing_keys)
        # target_spacing is the s_ref (could be median, unit, or per-case orig)
...
# ============================================================================
# Differentiable Resampler — R_α
# ============================================================================
# alpha = exp(beta), beta is the unconstrained learnable parameter.
# Target spacing at runtime: s_target = alpha * s_ref
# When beta = 0 (alpha = 1), s_target = s_ref → equivalent to fixed-spacing
# resampling.

class DifferentiableResampler(nn.Module):
    """Full-volume differentiable resampling with learnable spacing factor alpha.

    Parameters:
        s_ref : (3,) reference spacing — when alpha=1, target spacing = s_ref.
                Could be dataset median, unit (1,1,1), or per-case raw spacing
                (the latter handled at forward time, not buffered).
        learnable : if True, alpha is learnable; if False, alpha is frozen at 1.
    """

    def __init__(self, s_ref, learnable=True):
        super().__init__()
        self.register_buffer('s_ref', torch.tensor(s_ref, dtype=torch.float32))
        self.beta = nn.Parameter(torch.zeros(3), requires_grad=learnable)
    ...
    @property
    def spacing(self):
        return self.alpha * self.s_ref

    def get_spacing(self, s_ref_override=None):
        """Return current target spacing. If s_ref_override is given, use it."""
        ref = s_ref_override if s_ref_override is not None else self.s_ref
        return self.alpha * ref

    def compute_output_size(self, in_shape, s_orig, s_ref_override=None):
        """Compute resampled output size given raw shape and original spacing."""
        s_tgt = self.get_spacing(s_ref_override).detach()
        s_in = s_orig.detach().float()
        out = []
        for d in range(3):
            n = int(math.ceil((in_shape[d] - 1) * s_in[d].item()
                              / max(s_tgt[d].item(), 1e-6))) + 1
            out.append(max(n, 2))
        return tuple(out)

    def build_grid(self, in_shape, s_orig, out_size, s_ref_override=None):
        """Build sampling grid for F.grid_sample (differentiable w.r.t. alpha)."""
        dev = self.beta.device
        s_tgt = self.get_spacing(s_ref_override)
        s_in = s_orig.to(dev).float()
        grids = []
        for d in range(3):
            idx = torch.arange(out_size[d], device=dev, dtype=torch.float32)
            vox = idx * (s_tgt[d] / s_in[d])
            norm = 2.0 * vox / max(in_shape[d] - 1, 1) - 1.0
            grids.append(norm)
        gd, gh, gw = torch.meshgrid(grids[0], grids[1], grids[2], indexing='ij')
        grid = torch.stack([gw, gh, gd], dim=-1).unsqueeze(0)
        return grid

    def forward(self, image, s_orig, s_ref_override=None):
        """Resample image from s_orig to alpha * s_ref."""
        in_shape = image.shape[2:]
        out_size = self.compute_output_size(in_shape, s_orig, s_ref_override)
        grid = self.build_grid(in_shape, s_orig, out_size, s_ref_override)
        return F.grid_sample(image, grid, mode='bilinear',
                             padding_mode='border', align_corners=True)

    def resample_label(self, label, s_orig, s_ref_override=None):
        """Resample label with nearest-neighbor (no gradient)."""
        in_shape = label.shape[2:]
        out_size = self.compute_output_size(in_shape, s_orig, s_ref_override)
        grid = self.build_grid(in_shape, s_orig, out_size, s_ref_override).detach()
        out = F.grid_sample(label.float(), grid, mode='nearest',
                            padding_mode='border', align_corners=True)
        return out.long()

    def warp_back_to_original(self, volume, s_orig, orig_shape, mode='bilinear',
                              s_ref_override=None):
        """Inverse warp: from resampled space back to the raw original space."""
        dev = volume.device
        s_tgt = self.get_spacing(s_ref_override).detach()
        s_in = s_orig.to(dev).float()
        res_shape = volume.shape[2:]
        grids = []
        for d in range(3):
            idx = torch.arange(orig_shape[d], device=dev, dtype=torch.float32)
            vox = idx * (s_in[d] / s_tgt[d])
            norm = 2.0 * vox / max(res_shape[d] - 1, 1) - 1.0
            grids.append(norm)
        gd, gh, gw = torch.meshgrid(grids[0], grids[1], grids[2], indexing='ij')
        grid = torch.stack([gw, gh, gd], dim=-1).unsqueeze(0)
        return F.grid_sample(volume, grid, mode=mode,
                             padding_mode='border', align_corners=True)
...
class V10Model(nn.Module):
    """v10 model: differentiable resampler + shared patcher.

    Forward expects RAW (un-resampled) image and label, plus per-case raw
    spacing tensor `s_orig`. The model resamples internally via R_α, then
    invokes the same BaselinePatchModel logic.
    """

    def __init__(self, resampler, backbone, patch_size, augmenter=None,
                 oversized_factor=1.4):
        super().__init__()
        self.resampler = resampler
        self.patcher = BaselinePatchModel(
            backbone, patch_size, augmenter, oversized_factor)
        self.patch_size = patch_size
        self.oversized_patch = self.patcher.oversized_patch

    @property
    def backbone(self):
        return self.patcher.backbone

    def forward(self, image, label, s_orig, s_ref_override=None,
                num_patches=4, pos_ratio=0.5):
        with torch_autocast('cuda', enabled=False):
            image = image.float()
            label = label.long()
            image_res = self.resampler(image, s_orig, s_ref_override)
            label_res = self.resampler.resample_label(label, s_orig, s_ref_override)
        return self.patcher(image_res, label_res,
                            num_patches=num_patches, pos_ratio=pos_ratio)

    def inference_forward(self, x):
        return self.patcher.backbone(x)
...
# ============================================================================
# DARTS finite-difference α update
# ============================================================================

def _eval_val_dice_orig(model, val_loader, device, patch_size, total_stride,
                        post_process, alpha_ref_mode):
    """Compute mean val_dice_orig across the val set (used by DARTS)."""
    model.eval()
    dice_list = []
    with torch.inference_mode(), \
        ...
        for vd in val_loader:
            ...
            v_sp_orig = vd['spacing'].to(device).squeeze(0).float()
            s_ref_override = v_sp_orig if alpha_ref_mode == 'orig' else None

            vi_res = model.resampler(vi.float(), v_sp_orig, s_ref_override)
            vi_p, _ = pad_to_at_least(vi_res,
                model.resampler.resample_label(vl, v_sp_orig, s_ref_override),
                patch_size)
            ...
            orig_label = torch.from_numpy(
                orig_nii.get_fdata().astype(np.float32)
...
def _darts_update_alpha(model, val_loader, device, patch_size, total_stride,
                        post_process, args):
    """DARTS finite-difference update on β = log α.

    For each axis d:
      β_d ← β_d + ε  → eval val_dice_orig → D⁺
      β_d ← β_d - 2ε → eval val_dice_orig → D⁻
      β_d ← β_d + ε  (restore)
      g_d = (D⁺ - D⁻) / (2ε)
    Then β ← β + lr · g  (gradient ASCENT, maximize dice).
    """
    resampler = model.resampler
    baseline_d = _eval_val_dice_orig(
        model, val_loader, device, patch_size, total_stride,
        post_process, args.alpha_ref)
    grad = torch.zeros(3)
    for d in range(3):
        resampler.beta.data[d] += args.darts_epsilon
        d_plus = _eval_val_dice_orig(
            model, val_loader, device, patch_size, total_stride,
            post_process, args.alpha_ref)
        resampler.beta.data[d] -= 2 * args.darts_epsilon
        d_minus = _eval_val_dice_orig(
            model, val_loader, device, patch_size, total_stride,
            post_process, args.alpha_ref)
        resampler.beta.data[d] += args.darts_epsilon
        grad[d] = (d_plus - d_minus) / (2 * args.darts_epsilon)
    with torch.no_grad():
        resampler.beta.data += args.darts_lr * grad.to(resampler.beta.device)
    a = resampler.alpha.detach().cpu().numpy()
    sp = resampler.spacing.detach().cpu().numpy()
    print(f"  [DARTS] base_dice={baseline_d:.4f} | "
          f"grad=[{grad[0]:+.4f},{grad[1]:+.4f},{grad[2]:+.4f}] | "
          f"new α=[{a[0]:.4f},{a[1]:.4f},{a[2]:.4f}] | "
          f"spacing=[{sp[0]:.4f},{sp[1]:.4f},{sp[2]:.4f}]")


def parse_args():
    p = argparse.ArgumentParser(
        description='v10 — raw-axis pipeline with differentiable resampling (learnable α).')
    ...
    # ── α optimization ─────────────────────────────────────────────────
    p.add_argument('--alpha_ref', type=str, default='median',
                   choices=['median', 'unit', 'orig'],
                   help='Reference spacing s_ref. α=1 reduces to s_ref-resampling.')
    p.add_argument('--freeze_alpha', action='store_true',
                   help='Lock α=1 throughout training (sanity check vs baseline_v10).')
    p.add_argument('--alpha_lr', type=float, default=1e-4,
                   help='Adam LR for α (only used if backprop α and not frozen).')
    p.add_argument('--alpha_freeze_epochs', type=int, default=50,
                   help='Freeze α for first N epochs (warmup before α is updated).')
    # ── DARTS variant ──────────────────────────────────────────────────
    p.add_argument('--darts', action='store_true',
                   help='Use DARTS-style finite-difference α update (replaces backprop).')
    p.add_argument('--darts_lr', type=float, default=1e-3,
                   help='DARTS step size (gradient ASCENT on val_dice_orig).')
    p.add_argument('--darts_epsilon', type=float, default=0.1,
                   help='Finite-difference step size (in β-space, β=log α).')
    p.add_argument('--darts_interval', type=int, default=20,
                   help='Run DARTS update every N epochs (after freeze_epochs).')
    return p.parse_args()
...
def main():
    ...
    global_output_dir = args.out_dir or './outputs/lipo_obelisk_v10'
    ...
    # ── Resolve s_ref (reference spacing for α) ──────────────────────────────
    if args.alpha_ref == 'median':
        s_ref = np.array(target_pixdim, dtype=np.float32)
        print(f"[INFO] alpha_ref = MEDIAN target spacing: {s_ref}")
    elif args.alpha_ref == 'unit':
        s_ref = np.array([1.0, 1.0, 1.0], dtype=np.float32)
        print(f"[INFO] alpha_ref = UNIT (1,1,1) — fully decoupled from dataset")
    elif args.alpha_ref == 'orig':
        # Per-case raw spacing handled at runtime; here use median as buffer
        s_ref = np.array(target_pixdim, dtype=np.float32)
        print(f"[INFO] alpha_ref = ORIG (per-case raw spacing at runtime)")
    else:
        raise ValueError(f"Unknown alpha_ref: {args.alpha_ref}")

    # ── CPU transforms ────────────────────────────────────────────────────────
    # Pipeline: Load → CropFG(Otsu) → ZScore(raw, gt_zero) → StoreRawSpacingAndTarget
    # NO Spacingd here — resampling happens inside V10Model via R_α.
    # Same train and val transforms (Plan A: ZScore on raw, identical pipelines).
    train_transforms = Compose([
        ...
        StoreRawSpacingAndTargetd(keys=['image'], target_spacing=s_ref),
        EnsureTyped(keys=['image'], dtype=np.float32),
        EnsureTyped(keys=['label'], dtype=np.uint8),
        ToTensord(keys=['image', 'label', 'spacing', 'target_spacing']),
    ])
    ...
    for fold_info in folds_to_run:
        ...
        # Model: DifferentiableResampler + BaselinePatchModel
        backbone = DynUNet(
        ...
        # α optimization mode: backprop (default), frozen, or DARTS
        alpha_learnable = (not args.freeze_alpha) and (not args.darts)
        # NOTE: DARTS is also "not learnable via backprop" — α updated by FD
        if args.freeze_alpha:
            print("[INFO] α frozen at 1.0 (sanity-check mode)")
        elif args.darts:
            print("[INFO] α updated via DARTS finite-difference (no backprop)")
        else:
            print("[INFO] α updated via backprop on training loss")
        resampler = DifferentiableResampler(s_ref, learnable=alpha_learnable)
        model = V10Model(resampler, backbone, patch_size, augmenter).to(device)

        n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        n_alpha = sum(p.numel() for p in resampler.parameters() if p.requires_grad)
        print(f"[INFO] Params: {n_params / 1e6:.2f} M (α params: {n_alpha})")

        loss_fn = DiceCELoss(to_onehot_y=True, softmax=True,
                             squared_pred=True, include_background=False,
                             lambda_dice=0.5, lambda_ce=0.5)
        # Backbone optimizer (always SGD, same as baseline)
        backbone_optimizer = torch.optim.SGD(
            model.backbone.parameters(),
            lr=base_lr, momentum=0.99, nesterov=True, weight_decay=1e-5)
        # α optimizer (only if backprop α and not frozen)
        alpha_optimizer = None
        if alpha_learnable:
            alpha_optimizer = torch.optim.Adam(
                [resampler.beta], lr=args.alpha_lr, weight_decay=0)
        scaler = amp.GradScaler(enabled=torch.cuda.is_available())

        # For DARTS: keep beta frozen during inner training, update separately
        if args.darts:
            resampler.beta.requires_grad_(False)
        ...
        for epoch in range(max_epochs):
            ...
            # Unfreeze α after warmup (only for backprop α mode)
            if (alpha_optimizer is not None and
                    not resampler.beta.requires_grad and
                    (epoch + 1) > args.alpha_freeze_epochs):
                resampler.beta.requires_grad_(True)
                print(f"[INFO] Epoch {epoch + 1}: α unfrozen — backprop α now active")

            for batch in train_loader:
                images = batch['image'].to(device)            # raw (un-resampled)
                labels = batch['label'].to(device).long()
                s_orig = batch['spacing'].to(device).squeeze(0).float()
                s_ref_per_case = (batch['target_spacing'].to(device).squeeze(0).float()
                                  if args.alpha_ref == 'orig' else None)
                # alpha_ref='orig' means use this case's raw spacing as s_ref;
                # so target = α * s_ref (per-case). Otherwise use the buffered s_ref.
                # To implement 'orig' properly we pass s_orig as s_ref override:
                if args.alpha_ref == 'orig':
                    s_ref_override = s_orig
                else:
                    s_ref_override = None
                ...
                with amp.autocast(device_type='cuda', enabled=torch.cuda.is_available()):
                    logits, lbl_patches, _ = model(
                        images, labels, s_orig, s_ref_override=s_ref_override,
                        num_patches=args.num_patches, pos_ratio=args.pos_ratio)
                    if logits is None:
                        n_nan += 1
                        continue
                ...
                if alpha_optimizer is not None and resampler.beta.requires_grad:
            ...
            if (epoch + 1) % val_interval == 0:
                ...
                with torch.inference_mode(), \
                     amp.autocast('cuda', enabled=torch.cuda.is_available()):
                    for vd in val_loader:
                        vi = vd['image'].to(device)            # raw, un-resampled
                        vl = vd['label'].to(device).long()
                        v_sp_orig = vd['spacing'].to(device).squeeze(0).float()
                        s_ref_override = (v_sp_orig if args.alpha_ref == 'orig'
                                          else None)

                        # Resample full volume via R_α (eval mode, no grad)
                        vi_res = model.resampler(vi.float(), v_sp_orig, s_ref_override)
                        vl_res = model.resampler.resample_label(
                            vl, v_sp_orig, s_ref_override)

                        # Pad to oversized patch / total_stride for sliding window
                        vi_p, vl_p = pad_to_at_least(vi_res, vl_res, patch_size)
                        rs = list(vi_p.shape[2:])
                        ps_div, p2 = [], False
                        for ax in range(3):
                            ts = int(total_stride[ax])
                        ...
                        # Dice in raw original space via R_α^-1
                        label_path = vd['label_path'][0]
                        orig_nii = nib.load(label_path)
                ...
                # Read current α / spacing for logging
                alpha_now = resampler.alpha.detach().cpu().numpy()
                sp_now = resampler.spacing.detach().cpu().numpy()
                lr_alpha = (get_lr(alpha_optimizer)
                            if alpha_optimizer is not None else 0.0)

                elapsed = time.time() - t0
                msg = (f"Ep {epoch + 1:03d}/{max_epochs} | "
                       f"Loss={epoch_loss:.4f} | TrDice={epoch_dice:.4f} | "
                       f"VL={val_loss_ep:.4f} | D_res={val_dice_res:.4f} | "
                       f"D_orig={val_dice_orig:.4f} | LR={get_lr(backbone_optimizer):.5f} | "
                       f"α=[{alpha_now[0]:.3f},{alpha_now[1]:.3f},{alpha_now[2]:.3f}] | "
                       f"sp=[{sp_now[0]:.3f},{sp_now[1]:.3f},{sp_now[2]:.3f}] | "
                       f"{elapsed:.0f}s")
                if n_nan > 0:
                    msg += f" | nan={n_nan}"
                print(msg)
                ...
                with open(log_path, 'a', newline='') as fc:
                    csv.writer(fc).writerow([
                        ...
                        f'{val_dice_orig:.6f}',
                        f'{get_lr(backbone_optimizer):.6f}', f'{lr_alpha:.6f}',
                        f'{alpha_now[0]:.6f}', f'{alpha_now[1]:.6f}', f'{alpha_now[2]:.6f}',
                        f'{sp_now[0]:.6f}', f'{sp_now[1]:.6f}', f'{sp_now[2]:.6f}',
                        f'{elapsed:.2f}'])

                # ── DARTS α update (every darts_interval epochs after warmup) ──
                if (args.darts and (epoch + 1) > args.alpha_freeze_epochs
                        and ((epoch + 1) - args.alpha_freeze_epochs) % args.darts_interval == 0):
                    print(f"  [DARTS] Updating α at epoch {epoch + 1}...")
                    try:
                        _darts_update_alpha(
                            model, val_loader, device, patch_size, total_stride,
                            post_process, args)
                    except Exception as e:
                        print(f"  [DARTS] update failed: {e}")
                    model.train()
        ...
        with torch.inference_mode():
            for vd in val_loader:
                ...
                vl = vd['label'].to(device).long()
                v_sp_orig = vd['spacing'].to(device).squeeze(0).float()
                s_ref_override = (v_sp_orig if args.alpha_ref == 'orig' else None)

                # Resample full volume via R_α
                vi_res = model.resampler(vi.float(), v_sp_orig, s_ref_override)
                vl_res = model.resampler.resample_label(vl, v_sp_orig, s_ref_override)
                ...
                # Warp back to raw original space via R_α^-1
                label_path = vd['label_path'][0]
                orig_nii = nib.load(label_path)
                orig_data = orig_nii.get_fdata().astype(np.float32)
                orig_label = torch.from_numpy(orig_data).unsqueeze(0).unsqueeze(0).to(device)
