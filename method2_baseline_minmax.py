...
class RobustMinMaxNormalized(MapTransform):
    ...
    Method 2 (Baseline Min-Max):
        z = clip((x - p2) / (p98 - p2), 0, 1)
    where p2, p98 are computed on nonzero voxels per channel.
    """
        ...
        for key in self.keys:
            ...
            for c in range(img_np.shape[0]):
                ...
                if nonzero_mask.sum() > 0:
                    ...
                    result[c] = np.zeros_like(channel)

            if was_tensor:
                # Preserve MetaTensor if applicable
                if hasattr(img, 'meta'):
...
class NnUNetRandContrastd(MapTransform, RandomizableTransform):
    ...
    def __call__(self, data):
        ...
        for key in self.keys:
            img = d[key]
            if torch.is_tensor(img):
                orig_min, orig_max = img.min().item(), img.max().item()
                d[key] = torch.clamp(img * self._factor, orig_min, orig_max)
            else:
                orig_min, orig_max = img.min(), img.max()
                d[key] = np.clip(img * self._factor, orig_min, orig_max)
...
if __name__ == '__main__':
    ...
    global_output_dir = args.out_dir if args.out_dir else "./outputs/method2_baseline_minmax"
